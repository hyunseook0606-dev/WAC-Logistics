...
> 실 INV 기준 비항공 라인의 약 43%가 마스터 밖 변동비였고, Handling 단가 갭(150 vs 312)으로  
> 전면 자동화보다 원가·SOP·데이터 연계가 먼저라는 한계를 수치로 남김.

수치 표: [`검증-메트릭.md`](./검증-메트릭.md)

---

## 8. 프로젝트 마무리 (인턴 피드백 반영)

차장님 방향: 자체 견적 시스템 추가 빌드업보다 **원가·이익 이해**, **견적→INV 데이터 연계/SOP**, **W EXPRESS** 문제·개선 기획.

이 레포의 Instant Quote / Desk MVP는 **문제 정의·구조 검증·포폴 산출물**로 닫고,  
인턴 남은 기간 업무는 W EXPRESS 쪽으로 전환.
